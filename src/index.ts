...
interface Timestamp { from: string, to: string, text?: string }
interface Input { videoFile: ValidPath, outputDir: ValidPath & ValidDir, timestamps: Timestamp[] };
...
const validate = (args: any): Input => {
    ...
    return ({
        ...
        timestamps:
            (args.timestamps.length !== 0 ? args.timestamps : fs.readFileSync(args.timestampsFile).toString().split('\n'))
...
const buildCommand = (input: Input) =>
    input
        ...
        .map(ts => ({
            ...ts,
            outputVideoFile:
                path.join(input.outputDir, [
                    ts.from.replace(/\:/g, '_'),
                    path.basename(input.videoFile)
                ].join('-'),
                )
        }))
        .map((timestamp: Timestamp & { outputVideoFile: string }) =>
...
runCommand(
    buildCommand(validate(args)), args
);
